...
Component({
  ...
  methods: {
    onfullscreen() {
      this.data.playContext.requestFullScreen({
        direction: 90,
        success(res) {
    ...
    exitfullscreen() {
      this.data.playContext.exitFullScreen({
        success(res) {
    ...
    playerStateChange: function(e) {
      ...
      if (e.detail.code === 2004) {
        ...
        if (this.data.status === "loading") {
          console.warn('playerStateChange ok')
          this.setData({
            status: "ok"
          });
        }
  ...
  ready: function() {
    ...
    console.warn('player ready', this.data.agentInfo)
    ...
    this.setData({
      isScreen: Utils.isScreen(this.data.uid)
    })
  },
  moved: function() {
