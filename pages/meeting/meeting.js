...
Page({
  ...
  onReady: function() {
    ...
    // init agora channel
    console.warn('initAgoraChannel', uid, channel)
  ...
  syncLayout(media) {
    ...
    if (media.length === 2) {
      ...
      let newMedia = [];
      // 是否有屏幕共享,将屏幕共享放在第一个
      let shareMediaIndex = media.findIndex(item => Utils.isScreen(item.uid));
      if (shareMediaIndex) {
        media.unshift(media.splice(shareMediaIndex, 1)[0]);
      }
      // 将pusher放在最后
      let pusherMediaIndex = media.findIndex(item => item.type === 0);
      if (pusherMediaIndex) {
        media.push(media.splice(pusherMediaIndex, 1)[0])
      }
    }
    let sizes = this.layouter.getSize(media, this.data.camera);
    console.warn('syncLayout', media, media.length)
  ...
  onUnload: function() {
    ...
    wx.request({
      ...
      data: {
        ...
        type: '4',
        isOver: '', // 退出不销毁
        uid: `${app.globalData.uid}`.substr(6)
      },
      success(res) {
        console.warn('用户离开成功', res.data)
      },
      fail(err) {
        console.warn('用户离开失败', err)
      }
    })
  },
  ...
  initLayouter: function() {
    ...
    this.layouter = new Layouter(systemInfo.windowWidth, systemInfo.windowHeight - 80);
  },
  ...
  subscribeEvents: function(client) {
    ...
    client.on("stream-removed", e => {
      ...
      // 检测房间状态
      wx.request({
        url: `${app.globalData.baseUrl}/miniprogram/media/video/beatCheck`,
        method: 'POST',
        data: {
          roomId: app.globalData.channel
        },
  ...
  countTime() {
    ...
    setInterval(() => {
      ...
      var m = checkTime(parseInt(time / 60));
      var s = checkTime(parseInt(time - m * 60));
      this.setData({
        connectTime: `${m}:${s}`
      })
    }, 1000)
